...
app.use(
  cors({
    origin: [process.env.CLIENT_URL || "http://localhost:5173","http://10.242.3.66:5173"],
    credentials: true,
  })
);
...
app.get("/api/health", (req, res) => {
  res.status(200).json({ success: true, message: `NEW GENTLEMEN API is healthy ${process.env.CLIENT_URL}`, timestamp: new Date() });
});
app.get("/", (req, res) => {
